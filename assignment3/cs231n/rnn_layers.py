...
def word_embedding_forward(x, W):
    ...
    ##############################################################################
    # each element of x(N, T) is between(0, V)
    # each element is one row of W(,D)
    # thus all elements are masked to W[x,:] thus (N, T, D)
    out = W[x, :]
    cache = x, W
    ##############################################################################
    #                               END OF YOUR CODE                             #
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    # note shape of prev_h (N, H) and c (N, H)
    prev_h = h0
    c = np.zeros((N, H))
    cache = {}
    
    for i in range(T):
        prev_h, c, cache[i] = lstm_step_forward(x[:, i, :], prev_h, c, Wx, Wh, b)
        h[:, i, :] = prev_h
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
